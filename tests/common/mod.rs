...
// 3. No cleanup needed in ~/.plasma/ or any global location
//
...
/// Helper to start a Plasma daemon for testing
...
impl TestDaemon {
    ...
    fn start_with_mode(mode: DaemonMode) -> Self {
        let plasma_bin = env!("CARGO_BIN_EXE_plasma");
...
"#,
        ...
        let mut child = Command::new(plasma_bin)
            .arg("daemon")
            .arg("--config")
            .arg(&config_path)
            .env("PLASMA_STATE_DIR", &state_dir) // USE ISOLATED STATE DIR
            .spawn()
...
impl Drop for TestDaemon {
    fn drop(&mut self) {
        ...
        // No global state in ~/.plasma/ is touched!
...
/// Helper to start a Plasma regional server (Layer 2) for testing
...
"#,
        ...
        let child = Command::new(plasma_bin)
            .arg("server")
            .arg("--config")
            .arg(&config_path)
            .spawn()
