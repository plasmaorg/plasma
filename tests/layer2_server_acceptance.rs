...
//
// These tests verify the Plasma Protocol gRPC server functionality
...
async fn test_layer2_server_exists_not_found() {
    ...
    let mut client =
        plasma::plasma_protocol::proto::plasma_cache_client::PlasmaCacheClient::connect(
            server.grpc_url(),
        )
        .await
    ...
    let request = plasma::plasma_protocol::proto::ExistsRequest {
        hash: hash.to_string(),
    };
...
async fn test_layer2_server_put_and_exists() {
    ...
    // Create test data
    let test_data = b"Hello, Plasma Layer 2!";
    ...
    // PUT the data
    let put_requests = vec![plasma::plasma_protocol::proto::PutRequest {
        hash: hash.clone(),
    ...
    // Now check EXISTS
    let exists_request = plasma::plasma_protocol::proto::ExistsRequest { hash: hash.clone() };
...
async fn test_layer2_server_put_and_get() {
    ...
    // GET the data back
    let get_request = plasma::plasma_protocol::proto::GetRequest { hash: hash.clone() };
...
async fn test_layer2_server_get_not_found() {
    ...
    let get_request = plasma::plasma_protocol::proto::GetRequest {
        hash: hash.to_string(),
    };
...
async fn test_layer2_server_delete() {
    ...
    // DELETE the artifact
    let delete_request = plasma::plasma_protocol::proto::DeleteRequest { hash: hash.clone() };
    ...
    let exists_request2 = plasma::plasma_protocol::proto::ExistsRequest { hash: hash.clone() };
...
async fn test_layer2_server_get_stats() {
    ...
    let stats_request = plasma::plasma_protocol::proto::GetStatsRequest {
    ...
    // Get updated stats
    let stats_request2 = plasma::plasma_protocol::proto::GetStatsRequest {
        since_timestamp: None,
    };
...
async fn test_layer2_server_large_artifact() {
    ...
    for (i, chunk) in test_data.chunks(CHUNK_SIZE).enumerate() {
        put_requests.push(plasma::plasma_protocol::proto::PutRequest {
...
async fn test_layer2_server_isolation() {
    ...
    let exists_request1 = plasma::plasma_protocol::proto::ExistsRequest { hash: hash.clone() };
