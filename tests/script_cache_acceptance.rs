/// Acceptance tests for Plasma script caching
///
...
/// Helper to get the plasma binary path with unique cache dir
fn plasma_with_cache(cache_dir: &Path) -> Command {
    let mut cmd = Command::new(std::env!("CARGO_BIN_EXE_plasma"));
    cmd.env("PLASMA_CONFIG_CACHE_DIR", cache_dir);
    cmd
}
...
impl TestWorkspace {
    ...
    fn plasma(&self) -> Command {
        plasma_with_cache(self.cache_path())
    }
...
fn test_simple_node_script() {
    ...
    assert!(output.contains("Generated by Plasma"));
...
fn test_verbose_flag() {
    ...
    workspace
        ...
        .assert()
        .success()
        .stderr(predicate::str::contains("[plasma] Parsing annotations"))
        .stderr(predicate::str::contains("[plasma] Cache key:"))
        .stderr(predicate::str::contains("[plasma] Executing:"));
}
