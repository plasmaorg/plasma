...
fn create_oauth2_config(temp_dir: &TempDir, oauth_url: &str) -> PathBuf {
    let config_path = temp_dir.path().join("plasma.toml");
...
async fn test_oauth2_device_flow_login() {
    ...
    println!(
        "[plasma] Mock OAuth2 server started at: {}",
        server.base_url()
    );
    ...
    println!("[plasma] Login completed successfully");
    ...
    println!("[plasma] Authentication status verified");

    // Test logout
    provider2.logout().await.expect("Failed to logout");
    println!("[plasma] Logout completed successfully");

    // Leak provider2
    std::mem::forget(provider2);

    // Create new provider instance to verify logout
    let provider3 = plasma::auth::provider::AuthProvider::new(config.auth, config.url)
        .expect("Failed to create auth provider");
    ...
    println!("[plasma] Logout verified - no longer authenticated");
...
async fn test_oauth2_status_not_authenticated() {
    ...
    let provider = plasma::auth::provider::AuthProvider::new(config.auth, config.url)
        .expect("Failed to create auth provider");
