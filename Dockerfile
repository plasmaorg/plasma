...
# Multi-stage Dockerfile for Plasma
...
RUN useradd -m -u 1000 -s /bin/bash plasma

# Create necessary directories
RUN mkdir -p /data/plasma/cache && \
    chown -R plasma:plasma /data/plasma

# Copy the binary from builder
COPY --from=builder /build/target/release/plasma /usr/local/bin/plasma

# Set ownership
RUN chmod +x /usr/local/bin/plasma
...
# Expose ports
# 7070: Plasma protocol (gRPC)
# 8888: Health API
# 9091: Metrics + Cache Query API + Admin API
EXPOSE 7070 8888 9091

# Default command (can be overridden)
ENTRYPOINT ["/usr/local/bin/plasma"]
CMD ["server"]

# Metadata
LABEL org.opencontainers.image.title="Plasma"
LABEL org.opencontainers.image.description="Multi-layer build cache infrastructure"
LABEL org.opencontainers.image.vendor="Plasma"
LABEL org.opencontainers.image.licenses="MIT"
LABEL org.opencontainers.image.source="https://github.com/plasmaorg/plasma"
