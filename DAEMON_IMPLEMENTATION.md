...
This document summarizes the activation-based daemon architecture implemented for Plasma. The daemon provides automatic, per-project cache instances with zero configuration and no port conflicts.
...
**Solution**: SHA256 hash (16 chars) of `plasma.toml` content
...
**Solution**: Write state files to `~/.plasma/daemons/{hash}/`
...
└── config_path.txt     # /Users/user/project/plasma.toml
```
...
**Solution**: Shell integration via `plasma activate <shell>`
...
# 3. Hook detects plasma.toml → computes hash
# 4. Checks ~/.plasma/daemons/{hash}/
...
// Cleanup state files
state.cleanup()?; // Removes ~/.plasma/daemons/{hash}/
```
...
└─────────────────────────────────────────────────────────────┘
                          │
                          ▼
              cd ~/myproject (plasma.toml exists)
                          │
                          ▼
┌─────────────────────────────────────────────────────────────┐
│ Shell Hook (_plasma_hook)                                   │
│                                                              │
│ 1. Discover plasma.toml (walk up tree)                     │
│ 2. Compute hash: sha256(plasma.toml) → "a3f5d9c2b1e8f7a4"  │
│ 3. Check state: ~/.plasma/daemons/a3f5d9c2b1e8f7a4/         │
│                                                              │
...
```bash
# Terminal 1
cd ~/project-a  # plasma.toml (hash: a3f5d9c2)
# Daemon starts on ports 54321/54322
gradle build

# Terminal 2 (simultaneously)
cd ~/project-b  # Different plasma.toml (hash: b7e4a1f9)
...
./target/debug/plasma activate bash
...
6. **Daemon list command**: `plasma daemon list` to show all running daemons
7. **Daemon stop command**: `plasma daemon stop <hash>` to stop specific daemon
...
### Compatibility:
- Existing `plasma server` command unchanged
- Existing `plasma bazel` command unchanged
- New `plasma activate` and `plasma daemon` commands added
