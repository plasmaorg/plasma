...
Plasma is the foundational infrastructure for build caching, designed to be deployed and managed as a service by **Tuist**.

**Relationship to Tuist:**
> **Postgres is to Supabase** what **Plasma is to Tuist**

Just as Supabase deploys and manages Postgres databases for customers, Tuist will deploy and manage Plasma instances to provide build cache as a service. Plasma is tenant-agnostic infrastructure; Tuist handles all customer logic, billing, and orchestration.

Plasma provides a transparent, high-performance caching hierarchy to optimize build performance across different environments, supporting any build tool with remote caching capabilities including build systems (Gradle, Bazel, Nx, TurboRepo), compiler caches (sccache for Cargo/Rust), container build tools (BuildKit), and more.
...
- Plasma instance bound to build process lifecycle
...
- Dedicated Plasma instance deployed by Tuist for each customer/project
...
- Each customer gets their own Plasma instance(s) at Layer 2
...
**Plasma is tenant-agnostic:**
- Plasma doesn't understand "customers" or "projects"
- Authentication identifies the request, but Plasma treats all requests equally
...
**Key Design Decision: Build System Adapters + Unified Plasma Protocol**

**Two-Protocol Design:**
1. **Build Tool Protocols** (Layer 1 only): Gradle HTTP, Bazel gRPC, sccache S3, BuildKit registry, etc.
2. **Plasma Protocol** (inter-layer): Unified gRPC-based protocol for Layer 1 ↔ Layer 2 communication
...
│                    ▼                                    │
│        ┌─────────────────────┐                          │
│        │ Plasma Protocol     │  ◄── Unified gRPC        │
│        │ Client (gRPC)       │                          │
│        └─────────────────────┘                          │
└────────────────────┬────────────────────────────────────┘
                     │
            Plasma Protocol (gRPC)
          GET/PUT/EXISTS /v1/cache/{hash}
                     │
                     ▼
┌─────────────────────────────────────────────────────────┐
│ Layer 2: Regional Cache (Plasma Protocol Server)       │
│                                                         │
│        ┌─────────────────────┐                          │
│        │ Plasma Protocol     │                          │
│        │ Server (gRPC)       │                          │
...
4. Layer 1 Plasma client makes `Get(hash)` to Layer 2 (gRPC)
...
**Key Insights:**
- **Layer 1**: Runs build tool adapters, speaks Plasma protocol to upstream
- **Layer 2**: Only speaks Plasma protocol (no build tool knowledge)
...
**Benefits:**
- ✅ **Simpler Layer 2**: Only implements Plasma protocol
...
**Protocol Definition**: `proto/plasma.proto`
...
- **Port**: Default 7070 for Plasma protocol gRPC server
...
**Bazel**: Uses command-line flags (`--remote_cache`), so Plasma provides a wrapper command `plasma bazel` that:
...
plasma bazel -- build //... --config=release --jobs=8

# With custom cache directory:
plasma bazel --config-cache-dir=/custom/path -- build //...
```
...
**Token Validation (Plasma):**
- Plasma validates JWT signature locally using public key
...
```json
{
  ...
  "instance_id": "plasma_regional_us-east",
...
Plasma must support HTTP, gRPC, S3-compatible, and OCI registry protocols to accommodate all build tools.

## Analytics & Observability

### Design Philosophy: Pull-Based Model (Like Supabase)

**Tuist actively queries Plasma instances** rather than Plasma pushing telemetry elsewhere.

**Why Pull-Based:**
- ✅ Tuist controls query frequency (no overwhelming)
- ✅ No additional infrastructure (no telemetry pipeline, no Kafka/PubSub)
- ✅ Simpler security (Plasma doesn't need outbound access)
...
Plasma exposes **three HTTP APIs** for Tuist to consume:
...
```prometheus
# Cache performance
plasma_cache_hits_total 123456
plasma_cache_misses_total 7890
plasma_cache_hit_ratio 0.94

# Storage
plasma_cache_size_bytes 5368709120
plasma_cache_objects 45678

# Latency (histogram)
plasma_request_duration_seconds_bucket{le="0.005"} 9500
plasma_request_duration_seconds_bucket{le="0.01"} 9800
plasma_request_duration_seconds_bucket{le="0.025"} 9950

# Bandwidth
plasma_bandwidth_bytes_total{direction="upload"} 1073741824
plasma_bandwidth_bytes_total{direction="download"} 5368709120

# Upstream
plasma_upstream_requests_total{result="hit"} 5000
plasma_upstream_requests_total{result="miss"} 200

# Evictions
plasma_evictions_total 123
```
...
api_jwt_public_key_file = "/etc/plasma/api-public-key.pem"
...
```python
# Every 60 seconds
for instance in plasma_instances:
    # Pull metrics
...
- Storage: Compatible with Plasma's S3 storage layer
...
**Protocol Support:** Plasma must implement HTTP, gRPC, S3-compatible, and OCI registry APIs to support all build tools.

## Portable Recipes

### Overview

Plasma supports **portable recipes** - cross-platform JavaScript automation scripts that run on an embedded QuickJS runtime. Recipes provide two types of caching:
...
- **Custom APIs**: Plasma-specific APIs for caching and build operations
...
**Plasma-Specific APIs** (imported from `plasma:*` modules):

```javascript
// plasma:cache - Content-addressed caching APIs
import { runCached, needsRun } from 'plasma:cache';
...
await runCached(
  ...
  // Metadata for cache key computation + optional config overrides
  {
    ...
    // (defaults to plasma.toml discovered from script directory)
...
import { get, set, has } from 'plasma:kv';
...
import { get as getArtifact, put as putArtifact } from 'plasma:storage';

const artifact = await getArtifact(hash);  // Returns Uint8Array | null
await putArtifact(hash, new Uint8Array([...]));

// File utilities
import { glob, hashFile } from 'plasma:fs';
...
When a recipe calls `runCached()` or `needsRun()`, Plasma automatically discovers configuration:

1. **Start from recipe location**: Look for `plasma.toml` in the directory containing the recipe file
2. **Traverse up**: If not found, walk up parent directories (same as `plasma daemon`)
3. **Global fallback**: Use `~/.config/plasma/config.toml` if no project config found
...
2. Project `plasma.toml` (discovered from recipe directory)
3. Global `~/.config/plasma/config.toml` (lowest)

**Example directory structure:**
```
/home/user/projects/myproject/
├── plasma.toml                    # Project config (auto-discovered)
├── scripts/
│   └── build.recipe.js           # Recipe discovers ../plasma.toml
└── tools/
    └── deploy.recipe.js          # Also discovers ../../plasma.toml
```
...
```javascript
// build.recipe.js
import { runCached } from 'plasma:cache';
...
|--------|------|-------------|---------|
| `cacheDir` | string | Local cache directory | `.plasma-cache` |
...
**Benefits:**
- ✅ **Zero config**: Works out of the box with discovered `plasma.toml`
...
- [ ] Replace `Plasma.exec()` with proper cache-aware APIs (see API design discussion)
- [ ] Integrate cache operations with actual Plasma storage
...
When a configuration file is not explicitly specified via `--config`, Plasma automatically discovers it by:

1. **Traversing up from current directory**: Starting from the current working directory, Plasma looks for `plasma.toml` in each parent directory
2. **Global fallback**: If no project config is found, checks `~/.config/plasma/config.toml`
...
└── projects/
    └── myproject/
        ├── plasma.toml          # Project config (auto-discovered)
        └── src/
            └── subdir/          # Running from here finds ../plasma.toml
```

**Behavior:**
- `cd /home/user/projects/myproject/src/subdir && plasma daemon` → Uses `/home/user/projects/myproject/plasma.toml`
- `cd /home/user && plasma daemon` → Uses `/home/user/.config/plasma/config.toml`
- `plasma daemon --config /custom/path.toml` → Uses `/custom/path.toml` (explicit override)

### Design Philosophy
- **Single binary** with configurable behavior via flags
- **Unified upstream model**: S3, GCS, and other Plasma instances are all treated as "upstream" layers
...
- **Auto-discovery**: Zero configuration for simple use cases - just add `plasma.toml` to your project

### Configuration Quick Reference

**Three ways to configure Plasma (in precedence order):**

1. **Command-line flags** (highest priority)
   ```bash
   plasma daemon --config /custom/path.toml --log-level debug
   ```

2. **Environment variables**
   ```bash
   # Runtime flags
   export PLASMA_LOG_LEVEL=debug
   export PLASMA_VERBOSE=true
...
3. **Configuration file** (lowest priority)
   ...
   plasma daemon  # Finds ./plasma.toml or ../../plasma.toml or ~/.config/plasma/config.toml

   # Explicit path:
   plasma daemon --config /path/to/config.toml
   ```

**Key principles:**
- Config file auto-discovery: traverses up directory tree from cwd
- All flags support environment variables (`PLASMA_*` or `PLASMA_CONFIG_*`)
- CLI args override env vars, env vars override config file
- Zero configuration possible in many scenarios (CI, local dev)

### CLI Commands

**`plasma bazel`** - Wrapper for Bazel with automatic cache configuration
```bash
plasma bazel <BAZEL_ARGS>...
```
Starts Bazel gRPC adapter and injects `--remote_cache` flag automatically.

**`plasma activate`** - Activate shell integration for automatic daemon management
```bash
plasma activate <shell>     # Generate shell hook (bash, zsh, fish)
plasma activate --status    # Check/start daemon and export env vars
```

**`plasma daemon`** - Run long-lived local cache daemon (Layer 1 for development)
```bash
plasma daemon [OPTIONS]
```

**`plasma deactivate`** - Deactivate Plasma and optionally stop daemon
```bash
plasma deactivate [--stop-daemon]
```

**`plasma server`** - Run regional/cloud cache server (Layer 2)
```bash
plasma server [OPTIONS]
```

**`plasma config`** - Configuration utilities (validate, generate, show)
```bash
plasma config <validate|generate|show> [OPTIONS]
```

**`plasma health`** - Health check and diagnostics
```bash
plasma health [OPTIONS]
```

**`plasma run`** - Execute scripts with content-addressed caching
```bash
plasma run [OPTIONS] <SCRIPT> [-- <SCRIPT_ARGS>...]
```
Executes scripts (bash, node, etc.) with automatic caching based on KDL annotations in the script file.

**`plasma cache`** - Manage script cache
```bash
plasma cache <status|clean|list|stats> [OPTIONS]
```
Inspect and manage the script cache (status, clean, list entries, view statistics).

### Plasma Recipes

Plasma provides two types of **recipes** for cached automation:
...
- `#PLASMA input "glob"` - Track input files (supports globs, invalidates cache when files change)
- `#PLASMA input "file" hash=mtime|size|content` - Specify hash method (default: content)
- `#PLASMA output "path"` - Declare output paths to archive/restore (files or directories)
- `#PLASMA env "VAR_NAME"` - Track environment variable (invalidates cache when value changes)
- `#PLASMA depends "script.sh"` - Declare dependency on another script
- `#PLASMA depends "script.sh" use-outputs=#true` - Add dependency outputs as inputs
- `#PLASMA cache disable` - Disable caching for this script
- `#PLASMA cache ttl="7d"` - Set cache expiration (e.g., "2h", "7d", "30d")
- `#PLASMA cache key="custom"` - Override cache key computation
- `#PLASMA runtime node` - Override runtime (defaults to shebang)
- `#PLASMA runtime-arg "--max-old-space-size=4096"` - Pass argument to runtime
- `#PLASMA runtime-version` - Include runtime version in cache key
- `#PLASMA exec cwd="subdir"` - Set working directory for execution
- `#PLASMA exec timeout="5m"` - Set execution timeout
- `#PLASMA exec shell` - Execute via shell (enables shell features)
...
plasma run build.sh -- --production --verbose
...
# Check cache status
plasma cache status build.sh

# Clean cache for specific script
plasma cache clean build.sh
...
Portable recipes are **cross-platform automation scripts** written in TypeScript/JavaScript that run on Plasma's embedded Deno runtime. Unlike script recipes (which require platform-specific runtimes like bash, python, node), portable recipes are guaranteed to work on any operating system.

**Key Features:**
- **True cross-platform**: Same code works on Windows, macOS, and Linux
- **No external runtime**: Deno is embedded in Plasma binary
- **TypeScript support**: Full IDE features (autocomplete, type checking)
- **Plasma API**: Access to cache, execution, and glob primitives via `plasma:*` modules
...
```typescript
// build.recipe.ts
import { $, run, glob } from "plasma:runtime";
import { get, put, has } from "plasma:cache";
...
export async function install() {
  console.log("[plasma] Installing dependencies...");
  await $`npm install`;
}
...
export async function build() {
  ...
  console.log("[plasma] Building TypeScript...");
  await $`npm run build`;

  const files = await glob("dist/**/*");
  console.log(`[plasma] Generated ${files.length} output files`);
}
...
export async function test() {
  console.log("[plasma] Running tests...");
...
```typescript
// plasma:runtime - Execution primitives
import { $, run, glob, hashFile } from "plasma:runtime";
...
```typescript
// plasma:cache - Direct cache access
import { get, put, has } from "plasma:cache";
...
```typescript
// plasma:config - Runtime configuration
import { getConfig } from "plasma:config";
...
plasma run build.recipe.ts build

# Run default target (exported as default)
plasma run build.recipe.ts

# Install from registry
plasma recipe install @tuist/typescript-build

# Run installed recipe
plasma run @tuist/typescript-build build

# Publish to registry
plasma recipe publish build.recipe.ts
```

**File Naming Convention:**
- Script recipes: `build.sh`, `test.py`, `deploy.rb` (with `#PLASMA` annotations)
...
$ cat build.py
#!/usr/bin/env python3
#PLASMA input "tests/**/*.py"
import subprocess
subprocess.run(["pytest"])

$ plasma run build.py  # Works on Unix with Python installed

# Later: Convert to portable recipe for team
$ cat build.recipe.ts
import { $ } from "plasma:runtime";
...
$ plasma run build.recipe.ts test  # Works everywhere
```
...
Plasma uses an **activation-based daemon model** where each project automatically gets its own daemon instance when you `cd` into the project directory. This provides:
...
1. **Config Hash as Identity**: Each `plasma.toml` is hashed (SHA256, 16 chars) to create a unique daemon identifier
...
2. **Dynamic Port Allocation**: Daemons bind to port 0 (random available ports)
   ...
   - Actual ports written to state file: `~/.plasma/daemons/{hash}/ports.json`

3. **Shell Integration**: `plasma activate` hook runs on directory change
   - Detects `plasma.toml` (walks up directory tree)
...
4. **State Management**: Each daemon writes state to `~/.plasma/daemons/{hash}/`
   ```
   ~/.plasma/daemons/a3f5d9c2b1e8f7a4/
   ├── pid                 # Process ID
   ├── ports.json          # { "http": 54321, "grpc": 54322, "metrics": 9091 }
   └── config_path.txt     # /path/to/project/plasma.toml
   ```
...
# - Checks ~/.plasma/daemons/a3f5d9c2b1e8f7a4/ → not found
# - Spawns: plasma daemon --config plasma.toml &
# - Daemon binds to random ports (e.g., 54321, 54322)
# - Daemon writes ports.json
# - Shell hook exports:
export PLASMA_HTTP_URL=http://127.0.0.1:54321
export PLASMA_GRPC_URL=grpc://127.0.0.1:54322
...
──────────────────────  ────────────────────────────────────────
cd ~/project            1. Detect plasma.toml
                        2. Compute hash: a3f5d9c2
                        3. Check state: ~/.plasma/daemons/a3f5d9c2/
...
cd ~/other-project      1. Detect different plasma.toml
...
Ctrl+C on daemon        1. Receive SIGINT
                        ...
                        3. Remove ~/.plasma/daemons/a3f5d9c2/
                        4. Exit cleanly
```
...
```bash
# Developer workflow
tuist auth login              # Creates .plasma.token
cd ~/project                  # Daemon starts, loads .plasma.token
...
Plasma automatically detects the best storage backend for the environment with zero configuration required. This means the same configuration (or no configuration at all) works seamlessly in CI and local development.

#### Detection Logic

When Plasma starts, it automatically detects the runtime environment:
...
3. **Local/Other**: Falls back to filesystem storage
   - Uses `cache.dir` from config or default `/tmp/plasma-cache`

### Environment Variable Support

All configuration options and CLI flags can be overridden via environment variables using these conventions:

#### Configuration Overrides (`PLASMA_CONFIG_*`)

Configuration file values can be overridden using the `PLASMA_CONFIG_*` prefix:
...
| `daemon.socket` | `PLASMA_CONFIG_DAEMON_SOCKET` | `.plasma/socket` |

**Naming convention:**
- Nested config: `config.section.key` → `PLASMA_CONFIG_SECTION_KEY`
- Arrays: `upstream[0].url` → `PLASMA_CONFIG_UPSTREAM_0_URL`
- All uppercase, underscores separate words

#### Runtime Flags (`PLASMA_*`)

CLI flags and runtime options can be set via `PLASMA_*` (without `CONFIG`):

| CLI Flag | Environment Variable | Example |
|----------|---------------------|---------|
| `--config` | `PLASMA_CONFIG_PATH` | `/path/to/plasma.toml` |
| `--log-level` | `PLASMA_LOG_LEVEL` | `debug` |
| `--verbose` | `PLASMA_VERBOSE` | `true` |
| `--no-cache` | `PLASMA_NO_CACHE` | `true` |

#### Standard Environment Variable Fallbacks

Plasma respects standard environment variables where applicable:

| Purpose | Plasma Variable | Standard Fallback |
|---------|----------------|-------------------|
| S3 access key | `PLASMA_CONFIG_UPSTREAM_X_ACCESS_KEY` | `AWS_ACCESS_KEY_ID` |
| S3 secret key | `PLASMA_CONFIG_UPSTREAM_X_SECRET_KEY` | `AWS_SECRET_ACCESS_KEY` |
| S3 region | `PLASMA_CONFIG_UPSTREAM_X_REGION` | `AWS_REGION` |
| Auth token | `PLASMA_CONFIG_AUTH_TOKEN` | `PLASMA_TOKEN` |
...
jobs:
  build:
    ...
    steps:
      ...
      # Plasma automatically detects GitHub Actions and uses cache API
      - run: plasma bazel -- build //...
        env:
          PLASMA_TOKEN: ${{ secrets.PLASMA_TOKEN }}  # Optional: for upstream cache
```

**What happens:**
1. Plasma detects `ACTIONS_CACHE_URL` environment variable (auto-provided by GitHub)
2. Automatically uses GitHub Actions Cache as primary storage
3. Cache persists across workflow runs
4. No `plasma.toml` needed
...
build:
  script:
    - plasma bazel -- build //...
  variables:
    PLASMA_TOKEN: $CI_PLASMA_TOKEN
```

**Local Development** (no configuration needed):
```bash
# Set token once
export PLASMA_TOKEN=xxx
...
Plasma logs the detected storage backend at startup:

```
INFO plasma::storage - Detected GitHub Actions environment (ACTIONS_CACHE_URL present)
INFO plasma::storage - Using storage backend: github-actions
INFO plasma::storage - Cache URL: https://artifactcache.actions.githubusercontent.com/...
```

Or in local dev:

```
INFO plasma::storage - No CI environment detected
INFO plasma::storage - Using storage backend: filesystem
INFO plasma::storage - Cache directory: /tmp/plasma-cache
```
...
```toml
# plasma.toml in repository (optimized for Bazel)
[cache]
dir = "/mnt/build-cache"
max_size = "20GB"
...
plasma bazel -- build //...
plasma bazel -- test //...
plasma bazel -- build //... --config=release
```

**Layer 2 (Regional server with S3 upstream):**
```toml
# /etc/plasma/config.toml on server
[cache]
dir = "/data/plasma/cache"
max_size = "500GB"
eviction_policy = "lfu"
...
[build_tools]
enabled = []  # Empty - Layer 2 only speaks Plasma protocol
...
**What Layer 2 does:**
- ✅ Runs Plasma protocol gRPC server on port 7070
- ✅ Does NOT run Gradle/Bazel/Nx adapters
- ✅ Multi-tenant by default (all tenants use same Plasma protocol)
...
- Env: `PLASMA_CONFIG_CACHE_DIR=/tmp/cache`
...
Plasma checks both `PLASMA_CONFIG_*` and standard environment variables:

| Config Option | PLASMA_CONFIG_* | Standard Env Var |
|---------------|----------------|------------------|
| S3 access key | `PLASMA_CONFIG_S3_ACCESS_KEY` | `AWS_ACCESS_KEY_ID` |
| S3 secret key | `PLASMA_CONFIG_S3_SECRET_KEY` | `AWS_SECRET_ACCESS_KEY` |
| S3 region | `PLASMA_CONFIG_S3_REGION` | `AWS_REGION` |
...
2. On cache miss:
   - Bazel requests artifact from local Plasma adapter (gRPC)
   - Layer 1 adapter checks local RocksDB → MISS
   - Layer 1 queries Layer 2 (regional server via Plasma protocol) → ...
...
plasma config generate --template layer1 > plasma.toml

# Generate example server config
plasma config generate --template server > /etc/plasma/config.toml

# Validate config
plasma config validate /etc/plasma/config.toml

# Show effective configuration (merged from all sources)
plasma config show --config config.toml --config-upstream s3://override
```
...
- For CLI output (`println!`/`eprintln!`), use `plasma_prefix()` from `src/cli_utils.rs` which provides colored output
...
1. **Configuration File Discovery**:
   ...
   - Look for `plasma.toml` in each parent directory
   - Fallback to `~/.config/plasma/config.toml` if no project config found
   - Use the `config_discovery::discover_config()` function

2. **Environment Variable Support**:
   - ALL CLI flags/options must support environment variables with `PLASMA_` prefix
   - Configuration overrides use `PLASMA_CONFIG_*` prefix
   - Examples:
     - CLI flag `--log-level` → env var `PLASMA_LOG_LEVEL`
     - Config `cache.dir` → env var `PLASMA_CONFIG_CACHE_DIR`
     - Config `upstream[0].url` → env var `PLASMA_CONFIG_UPSTREAM_0_URL`
...
4. **Naming Conventions**:
   - Runtime flags: `PLASMA_{FLAG_NAME}` (e.g., `PLASMA_VERBOSE`, `PLASMA_LOG_LEVEL`)
   - Config overrides: `PLASMA_CONFIG_{SECTION}_{KEY}` (e.g., `PLASMA_CONFIG_CACHE_DIR`)
   - Nested config: Use underscores (e.g., `observability.log_level` → `PLASMA_CONFIG_OBSERVABILITY_LOG_LEVEL`)
   - Arrays: Use index (e.g., `upstream[0].url` → `PLASMA_CONFIG_UPSTREAM_0_URL`)

5. **Standard Fallbacks**:
   - Support standard env vars where applicable (e.g., `AWS_ACCESS_KEY_ID`, `AWS_REGION`)
   - Check Plasma-specific vars first, then fall back to standard vars
...
- **Tuist is the customer-facing layer** - Plasma is infrastructure
...
Plasma now supports peer-to-peer cache sharing on local networks, providing ultra-low latency cache access for teams working in the same office or network.
...
**✅ CLI Management**
- `plasma p2p list` - List discovered peers
- `plasma p2p status` - Show P2P status and stats
- `plasma p2p approve <peer>` - Approve peer access
- `plasma p2p deny <peer>` - Deny peer access
- `plasma p2p clear` - Clear all consents
...
**mDNS Discovery**
- Service type: `_plasma._tcp.local.`
...
$ plasma daemon
[plasma] P2P cache sharing is enabled
[plasma] Advertising P2P service as 'plasma-macbook' on port 7071
[plasma] Discovered peer: dev-bob at 192.168.1.15:7071

# Terminal 2 (Developer B)
$ plasma daemon
[plasma] Discovered peer: dev-alice at 192.168.1.10:7071

# Developer A builds
$ gradle build
[plasma] Cache MISS locally
[plasma] Querying 1 P2P peer in parallel
[plasma] P2P HIT from dev-bob (2.8ms, 1024 bytes)

# Developer B sees notification (first time only)
┌────────────────────────────────────┐
│ Plasma Cache Request               │
...
- All the logs should be prefixed with [plasma] consistently throughout the CLI
- Follow XDG conventions for the directories where Plasma stores state in the system
