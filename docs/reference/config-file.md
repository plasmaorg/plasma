...
Complete reference for the `.plasma.toml` configuration file.

## File Location

- **Project config**: `.plasma.toml` in project root
- **System config**: `/etc/plasma/config.toml`
- **Custom**: Specify with `--config <path>`

## Environment Variable Expansion

Plasma supports environment variable expansion in configuration files using shell-like syntax:
...
- **`${VAR}`** - Required variable (Plasma will error if not set)
...
[cache]
dir = "${CI_PROJECT_DIR:-/tmp}/.plasma/cache"
```
...
> [!IMPORTANT]
> Variables are expanded when the configuration file is loaded. If a required variable (`${VAR}`) is not set, Plasma will exit with an error message.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `dir` | string | `.plasma/cache` | Cache directory path |
...
[p2p]
enabled = true
secret = "my-team-secret-2024-plasma"
...
**Note:** Set `layer = "regional"` for Layer 2 servers that serve the Plasma gRPC protocol. Layer 1 (local daemons) should use the default `layer = "local"` or omit the section entirely.
...
export TUIST_CONFIG_CACHE_DIR=/tmp/plasma-cache
