...
Plasma uses structured logging via the `tracing` crate to provide consistent, parseable logs that are useful both for development and production monitoring.

## Log Formats

Plasma supports three output formats:
...
```
2025-10-25T17:53:04.123456Z INFO(plasma): service=xcode.cas operation=get status=success object_id=0164f7fc size_bytes=2048: cache hit
```
...
Set the `PLASMA_LOG_FORMAT` environment variable:

```bash
# Pretty format (default)
PLASMA_LOG_FORMAT=pretty plasma xcodebuild -- ...

# Compact format
PLASMA_LOG_FORMAT=compact plasma xcodebuild -- ...

# JSON format
PLASMA_LOG_FORMAT=json plasma xcodebuild -- ...
```

In CI environments, Plasma automatically defaults to compact format when the `CI` environment variable is set.
...
```bash
# Info level (default)
RUST_LOG=info plasma xcodebuild -- ...

# Debug level (verbose)
RUST_LOG=debug plasma xcodebuild -- ...

# Only warnings and errors
RUST_LOG=warn plasma xcodebuild -- ...
```
...
```
2025-10-25T17:53:04.234567Z INFO(plasma): service=xcode.cas operation=get status=miss object_id=abc123: cache miss
```

### Cache Write

```
2025-10-25T17:53:04.345678Z INFO(plasma): service=xcode.cas operation=put status=success object_id=0164f7fc size_bytes=2048: object stored
```

### Batch Operation

```
2025-10-25T17:53:04.456789Z INFO(plasma): service=bazel.cas operation=find_missing status=success instance=main blob_count=50 missing_count=5: check completed
```
