...
This document explains the optimizations applied to speed up Plasma's Docker builds from 20+ minutes to ~2-5 minutes.
...
```bash
DOCKER_BUILDKIT=1 docker build -t plasma:latest .
```
...
2. **Clear cache and rebuild:**
   ...
   docker build --no-cache -f Dockerfile.fast -t plasma:latest .
   ```
