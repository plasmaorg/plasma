...
Plasma provides automatic cache eviction to prevent unbounded disk usage. When the cache exceeds the configured `max_size`, Plasma automatically removes objects based on the configured eviction policy.

## Configuration

Configure eviction in your `plasma.toml`:
...
3. **Target**: Plasma evicts until the cache is at 90% of `max_size` (configurable via `target_ratio`)
...
Plasma tracks the following metadata for each cached object:
...
Plasma logs eviction activity at the INFO level:

```
INFO [plasma] Eviction manager initialized: policy=lfu, max_size=5120MB, target_ratio=0.9
INFO [plasma] Eviction complete: evicted 42 objects (128 MB) in 25ms
```
...
export PLASMA_CONFIG_CACHE_MAX_SIZE=10GB

# Override eviction policy
export PLASMA_CONFIG_CACHE_EVICTION_POLICY=lru

# Override default TTL
export PLASMA_CONFIG_CACHE_DEFAULT_TTL=14d
```
...
PlasmaCache* cache = plasma_cache_init("/path/to/cache");
