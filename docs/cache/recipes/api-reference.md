...
Plasma recipes have access to Node.js-compatible APIs (via LLRT) and Plasma-specific APIs for caching and file operations.
...
These modules provide Plasma-specific functionality:

- **`plasma:cache`** - Content-addressed caching
- **`plasma:fs`** - File utilities (glob, hashing)
- **`plasma:kv`** - Key-value storage for cache metadata

---

## Plasma Global Object

The `Plasma` global object is automatically available in all recipes and provides core functionality for file operations, process execution, and caching.

### `Plasma.readFile(path)`
...
**Example:**
```javascript
const data = await Plasma.readFile("config.json");
const text = new TextDecoder().decode(data);
```

---

### `Plasma.writeFile(path, data)`
...
await Plasma.writeFile("output.txt", data);
```
...
**Example:**
```javascript
const tsFiles = await Plasma.glob("src/**/*.ts");
console.log(`Found ${tsFiles.length} TypeScript files`);
```

---

### `Plasma.exec(command, args)`
...
**Example:**
```javascript
const exitCode = await Plasma.exec("npm", ["run", "build"]);
if (exitCode !== 0) {
...
**Example:**
```javascript
const hash = await Plasma.hashFile("package-lock.json");
...
- `Plasma.cache.get(hash)` - Get artifact from cache
- `Plasma.cache.put(hash, data)` - Store artifact in cache
- `Plasma.cache.has(hash)` - Check if artifact exists

> [!NOTE]
> These are low-level APIs. Most recipes should use `runCached()` from `plasma:cache` instead.
...
- `options` (Object): Cache configuration
  ...
  - `cacheDir` (string, optional): Override cache directory (default: `.plasma/cache`)
...
```javascript
import { needsRun } from 'plasma:cache';
...
if (shouldRun) {
  ...
  const exitCode = await Plasma.exec("cargo", ["build", "--release"]);
  if (exitCode !== 0) {
    throw new Error("Build failed");
  }
} else {
...
```javascript
import { hashFile } from 'plasma:fs';
...
> [!NOTE]
> LLRT's `child_process` module is provided for Node.js compatibility but may have different behavior than Node.js. For simpler process execution, consider using the global `Plasma.exec()` function.
...
**Alternative: Plasma.exec() (recommended)**

The global `Plasma.exec()` function provides simpler process execution:

```javascript
// Simpler process execution via Plasma global
const exitCode = await Plasma.exec("npm", ["install"]);
...
> [!TIP]
> `Plasma.exec()` currently returns only the exit code. Support for capturing stdout/stderr may be added in a future release.
...
When recipes call `runCached()` or `needsRun()`, Plasma automatically discovers configuration:

1. **Start from recipe location**: Look for `plasma.toml` in the directory containing the recipe
2. **Traverse up**: Walk up parent directories until `plasma.toml` is found
...
3. Global `~/.config/plasma/config.toml`

**Example:**

```javascript
// Uses discovered plasma.toml automatically
await runCached(
  async () => { /* action */ },
...
- [Standard Recipes](/cache/recipes/standard/) - Script recipes with PLASMA annotations
- [Configuration Reference](/reference/config-file) - Plasma configuration options
