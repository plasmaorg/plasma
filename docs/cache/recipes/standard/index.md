...
Standard recipes are scripts in any language (bash, node, python, ruby, etc.) that use [PLASMA annotations](/cache/recipes/standard/annotations) for content-addressed caching. Unlike [portable recipes](/cache/recipes/portable/) (which run in Plasma's embedded JavaScript runtime), standard recipes:
...
- ✅ Work with existing scripts - just add PLASMA annotations

While Plasma integrates with build tools like Gradle and Bazel, many build workflows include custom scripts that aren't covered by those tools. Standard recipes fill this gap.
...
With standard recipes, you declare inputs and outputs using [PLASMA annotations](/cache/recipes/standard/annotations), and Plasma handles the rest.

## Standard vs Portable Recipes

| Feature | Standard Recipes | [Portable Recipes](/cache/recipes/portable/) |
|---------|------------------|------------------|
| **Runtime** | System (bash, node, python) | Embedded (QuickJS in Plasma) |
| **Languages** | Any with shebang | JavaScript only |
| **Dependencies** | Requires runtime installed | Zero dependencies |
| **Distribution** | Managed manually | Managed by Plasma (via `@` syntax) |
| **Caching** | [PLASMA annotations](/cache/recipes/standard/annotations) | [PLASMA annotations](/cache/recipes/standard/annotations) + runtime JS APIs |

## How It Works

1. **Add annotations to your script** - Declare inputs, outputs, and environment variables using `#PLASMA` comments
2. **Run with `plasma run`** - Execute your script through Plasma
3. **Automatic caching** - Plasma computes a cache key based on script content, inputs, and env vars
...
Plasma provides compact, single-line output:
