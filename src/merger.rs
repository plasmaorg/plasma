...
impl MergedExecConfig {
    ...
    pub fn merge(args: &ExecArgs, file_config: Option<PlasmaConfig>) -> Self {
...
impl MergedServerConfig {
    /// Merge configuration from CLI args and config file
    /// Precedence: CLI > env (already handled by clap) > config file > defaults
    pub fn merge(args: &ServerArgs, file_config: Option<PlasmaConfig>) -> Self {
        let file = file_config.unwrap_or_default();
        ...
        Self {
            ...
            plasma_bind: args.config_plasma_bind.clone(),
