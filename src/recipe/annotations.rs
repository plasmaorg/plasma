...
///
/// Parses #PLASMA directives from scripts to extract cache configuration.
...
pub fn parse_annotations(script_path: &Path) -> Result<ScriptAnnotations> {
    ...
    if annotations.runtime.is_empty() {
        return Err(anyhow!(
            "Runtime not specified. Add runtime to shebang (#!/usr/bin/env -S plasma run bash), \
             use #PLASMA runtime directive, or pass as CLI arg (plasma run bash script.sh)"
        ));
    }
...
///
/// Returns the appropriate comment prefix(es) for PLASMA annotations based on the runtime.
...
///
/// Example: #!/usr/bin/env -S plasma run bash -x
/// Or: #!/usr/bin/env -S plasma run  (runtime comes from #PLASMA runtime directive)
fn parse_shebang(line: &str) -> Result<(String, Vec<String>)> {
    ...
    // Find "plasma" and "run"
    let plasma_idx = parts
        .iter()
        .position(|&p| p == "plasma")
        .ok_or_else(|| anyhow!("Shebang must contain 'plasma run'"))?;

    let run_idx = plasma_idx + 1;
    if parts.get(run_idx) != Some(&"run") {
        return Err(anyhow!("Shebang must be 'plasma run [runtime]'"));
    }

    let runtime_idx = run_idx + 1;

    // Runtime is optional - can be specified via #PLASMA runtime directive
...
mod tests {
    ...
    fn test_parse_shebang() {
        let shebang = "#!/usr/bin/env -S plasma run bash";
        let (runtime, args) = parse_shebang(shebang).unwrap();
        assert_eq!(runtime, "bash");
        assert_eq!(args.len(), 0);

        let shebang = "#!/usr/bin/env -S plasma run node --experimental-modules";
        let (runtime, args) = parse_shebang(shebang).unwrap();
    ...
    fn test_parse_bash_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run bash
#PLASMA input "src/**/*.sh"
#PLASMA output "dist/"
#PLASMA env "PATH"
...
"#,
    ...
    fn test_parse_python_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run python3
#PLASMA input "src/**/*.py"
#PLASMA output "dist/"
#PLASMA cache ttl="1h"
...
"#,
    ...
    fn test_parse_node_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run node
//PLASMA input "src/**/*.js"
//PLASMA output "dist/"
//PLASMA env "NODE_ENV"
...
"#,
    ...
    fn test_parse_clojure_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run bb
;PLASMA input "src/**/*.clj"
;PLASMA output "target/"
;PLASMA env "JAVA_HOME"
...
"#,
    ...
    fn test_parse_lua_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run lua
--PLASMA input "src/**/*.lua"
--PLASMA output "dist/"
...
"#,
    ...
    fn test_parse_sql_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run psql
--PLASMA input "migrations/*.sql"
--PLASMA output "schema.dump"
--PLASMA env "DATABASE_URL"
...
"#,
    ...
    fn test_parse_haskell_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run runghc
--PLASMA input "src/**/*.hs"
--PLASMA output "dist/"
...
"#,
    ...
    fn test_parse_erlang_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run escript
%PLASMA input "src/**/*.erl"
%PLASMA output "_build/"
...
"#,
    ...
    fn test_parse_ruby_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run ruby
#PLASMA input "lib/**/*.rb"
#PLASMA input "Gemfile"
#PLASMA output "pkg/"
#PLASMA env "BUNDLE_PATH"
...
"#,
    ...
    fn test_parse_perl_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run perl
#PLASMA input "lib/**/*.pm"
#PLASMA output "blib/"
...
"#,
    ...
    fn test_parse_r_script() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run rscript
#PLASMA input "data/*.csv"
#PLASMA output "results/"
...
"#,
    ...
    fn test_parse_php_script_with_hash() {
        ...
        std::fs::write(
            &script_path,
            r#"#!/usr/bin/env -S plasma run php
#PLASMA input "src/**/*.php"
#PLASMA output "vendor/"
