//! Structured logging utilities for Plasma
//!
...
/// Custom formatter that shows "plasma" instead of full module path
struct PlasmaFormatter {
    with_ansi: bool,
}

impl<S, N> FormatEvent<S, N> for PlasmaFormatter
where
    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        // Write level with plasma in parentheses and color if ansi enabled
        if self.with_ansi {
            ...
            write!(writer, "{}{:5}(plasma)\x1b[0m: ", level_style, meta.level())?;
        } else {
            write!(writer, "{:5}(plasma): ", meta.level())?;
        }
...
impl LogFormat {
    /// Parse from environment variable (PLASMA_LOG_FORMAT)
    pub fn from_env() -> Self {
        match std::env::var("PLASMA_LOG_FORMAT")
            .unwrap_or_default()
            .to_lowercase()
            .as_str()
        {
...
/// - `PLASMA_LOG_FORMAT`: Set format ("pretty", "compact", "json")
...
/// PLASMA_LOG_FORMAT=json cargo run
///
    ...
    match format {
        LogFormat::Pretty => {
            tracing_subscriber::registry()
                ...
                .with(
                    fmt::layer()
                        .event_format(PlasmaFormatter { with_ansi: true })
        ...
        LogFormat::Compact => {
            tracing_subscriber::registry()
                ...
                .with(
                    fmt::layer()
                        .event_format(PlasmaFormatter { with_ansi: false })
                        .with_writer(io::stderr),
                )
                .init();
        }
