    ...
    fn create(oauth2_config: &OAuth2Config, root_url: Option<String>) -> Result<Self> {
        ...
        match oauth2_config.storage.as_str() {
            ...
            "file" => {
                ...
                // Stores tokens in: $XDG_DATA_HOME/plasma/ or ~/.local/share/plasma/
                let storage = Arc::new(
                    FileStorage::new("plasma")
...
impl AuthProvider {
    ...
    ///
    /// Priority:
    /// 1. PLASMA_AUTH_PROVIDER env var (explicit override)
    /// 2. PLASMA_TOKEN env var present → use token
    ...
    fn detect_provider(&self) -> Result<ConfigAuthProvider, AuthenticationError> {
        ...
        if let Ok(provider_str) = std::env::var("PLASMA_AUTH_PROVIDER") {
            return match provider_str.to_lowercase().as_str() {
                ...
                _ => Err(AuthenticationError::ConfigError(format!(
                    "Invalid PLASMA_AUTH_PROVIDER value: '{}'. Must be 'token' or 'oauth2'",
                    provider_str
                ))),
            };
        }

        // 2. Check if PLASMA_TOKEN is set → use token auth
        if std::env::var("PLASMA_TOKEN").is_ok() {
    ...
    fn get_token_from_config(&self) -> Result<String, AuthenticationError> {
        ...
        // Fall back to convention-based environment variable (PLASMA_TOKEN)
        if let Ok(token) = std::env::var("PLASMA_TOKEN") {
            return Ok(token);
        }
