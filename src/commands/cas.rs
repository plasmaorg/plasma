/// `plasma cas` command implementation
///
...
async fn get(
    ...
    if verbose && !json {
        println!("{} Retrieving blob: {}", plasma_prefix(), hash);
    }
        ...
        match output_path {
            Some(path) => {
                ...
                if json {
                    ...
                    println!("{} Written to: {}", plasma_prefix(), path);
                }
            }
            None => {
                // Write to stdout
...
async fn put(
    ...
    if let Some(expected) = expected_hash {
        ...
        if verbose && !json {
            println!("{} Hash verified: {}", plasma_prefix(), expected);
        }
    } else if verbose && !json {
        println!("{} Computed hash: {}", plasma_prefix(), computed_hash);
    }

    if verbose && !json {
        println!("{} Storing blob: {}", plasma_prefix(), computed_hash);
    }
    ...
    if json {
        ...
        println!("{} Blob stored: {}", plasma_prefix(), computed_hash);
        println!("{} Size: {} bytes", plasma_prefix(), data_len);
    }
...
async fn exists(storage: &FilesystemStorage, hash: &str, json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("{} Blob exists: {}", plasma_prefix(), hash);
        std::process::exit(0);
    } else {
        println!("{} Blob not found: {}", plasma_prefix(), hash);
        std::process::exit(1);
    }
}
...
async fn delete(storage: &FilesystemStorage, hash: &str, force: bool, json: bool) -> Result<()> {
    ...
    if !force && !json {
        ...
        if !response.trim().eq_ignore_ascii_case("y") {
            println!("{} Deletion cancelled.", plasma_prefix());
            return Ok(());
        }
    }
    ...
    if json {
        ...
        println!("{} Blob deleted: {}", plasma_prefix(), hash);
    }
...
async fn info(storage: &FilesystemStorage, hash: &str, json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("{} Blob: {}", plasma_prefix(), hash);
        println!(
            "{} Size: {} bytes ({:.2} MB)",
            plasma_prefix(),
            size,
