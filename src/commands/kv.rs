/// `plasma kv` command implementation
///
...
async fn get(
    ...
    if verbose && !json {
        println!("{} Retrieving key: {}", plasma_prefix(), key);
    }
...
async fn put(
    ...
    if verbose && !json {
        println!("{} Storing key: {}", plasma_prefix(), key);
    }
    ...
    if json {
        ...
        println!("{} Key stored: {}", plasma_prefix(), key);
...
async fn exists(storage: &FilesystemStorage, key: &str, json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("{} Key exists: {}", plasma_prefix(), key);
        std::process::exit(0);
    } else {
        println!("{} Key not found: {}", plasma_prefix(), key);
        std::process::exit(1);
    }
}
...
async fn delete(storage: &FilesystemStorage, key: &str, force: bool, json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("{} Key deleted: {}", plasma_prefix(), key);
    }
