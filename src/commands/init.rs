...
pub fn run(args: InitArgs) -> Result<()> {
    println!("🚀 Plasma Initialization\n");

    // Check if plasma.toml already exists
    if std::path::Path::new("plasma.toml").exists() {
        print!("⚠️  plasma.toml already exists. Overwrite? [y/N] ");
        io::stdout().flush()?;
    ...
    let cache_dir = if let Some(dir) = args.cache_dir {
        ...
        ".plasma/cache".to_string()
    } else {
        prompt_with_default("Cache directory", ".plasma/cache")?
    };
    ...
    // Generate plasma.toml content
    let mut config = format!(
        r#"# Plasma configuration
# See https://github.com/plasmaorg/plasma for more information
...
"#,
    ...
    // Write plasma.toml
    fs::write("plasma.toml", &config).context("Failed to write plasma.toml")?;
