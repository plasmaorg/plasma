...
pub fn run(args: DoctorArgs) -> Result<()> {
    println!("🔍 Plasma Doctor - System Configuration Check\n");

    let mut all_ok = true;

    // Check 1: Plasma binary
    if let Ok(exe_path) = env::current_exe() {
        println!("✅ Plasma binary found: {}", exe_path.display());
        if args.verbose {
            println!("   Version: {}", env!("CARGO_PKG_VERSION"));
        }
    } else {
        println!("❌ Could not determine Plasma binary path");
        all_ok = false;
    }
    ...
    if let Some(shell_name) = &shell {
        ...
        if integration_status {
            ...
            match shell_name.as_str() {
                "bash" => {
                    println!("   echo 'eval \"$(plasma activate bash)\"' >> ~/.bashrc");
                    println!("   source ~/.bashrc");
                }
                "zsh" => {
                    println!("   echo 'eval \"$(plasma activate zsh)\"' >> ~/.zshrc");
    ...
    let state_dir = plasma::xdg::daemon_state_dir();
    ...
    if let Ok(current_dir) = env::current_dir() {
        if let Ok(Some(config_path)) = crate::config_discovery::discover_config(&current_dir) {
            ...
            println!("ℹ️  No plasma.toml found in current directory");
            if args.verbose {
                println!("   Run 'plasma init' to create plasma.toml");
            }
        }
    }
    ...
    println!();
    if all_ok {
        println!("✅ All checks passed! Plasma is properly configured.");
    } else {
    ...
    if let Some(config_path) = config_file {
        if config_path.exists() {
            if let Ok(contents) = fs::read_to_string(&config_path) {
                let has_integration = contents.contains("plasma activate");

                if verbose {
                    println!("   Config file: {}", config_path.display());
                    if has_integration {
                        println!("   Found 'plasma activate' in config");
                    }
                }
