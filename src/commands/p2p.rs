...
pub async fn run(args: P2pArgs) -> Result<()> {
    ...
    let config = load_config_with_discovery(args.config.as_deref())?
        .context("No configuration file found. Run 'plasma init' to create one.")?;

    // Check if P2P is enabled
    if !config.p2p.enabled {
        anyhow::bail!(
            "P2P is not enabled in configuration. Set p2p.enabled = true in your plasma.toml"
        );
    }
...
async fn list_peers(config: &PlasmaConfig, verbose: bool, json: bool) -> Result<()> {
...
async fn show_status(config: &PlasmaConfig, json: bool) -> Result<()> {
    // Initialize P2P manager
    let p2p = P2PManager::new(config.p2p.clone()).await?;
    p2p.start().await?;
...
async fn approve_peer(config: &PlasmaConfig, peer: &str, permanent: bool) -> Result<()> {
...
async fn deny_peer(config: &PlasmaConfig, peer: &str) -> Result<()> {
    let consent_manager = Arc::new(ConsentManager::new(Arc::new(config.p2p.clone()))?);
...
async fn clear_consents(config: &PlasmaConfig, force: bool) -> Result<()> {
    if !force {
