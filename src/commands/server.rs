...
pub async fn run(args: ServerArgs) -> Result<()> {
    ...
    // Keep a copy of the full config for Plasma protocol settings
    ...
    if is_regional {
        ...
        info!("  - PlasmaCache service (Exists, Get, Put, Delete, GetStats)");

        handles.push(tokio::spawn(async move {
            let plasma_service = PlasmaCacheService::new(regional_storage);

            Server::builder()
                .add_service(PlasmaCacheServer::new(plasma_service))
