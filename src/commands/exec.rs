...
pub async fn run(args: ExecArgs) -> Result<()> {
    ...
    if args.command.is_empty() {
        anyhow::bail!("No command specified. Usage: plasma exec -- <command>");
    }
    ...
    info!("Starting Plasma exec mode");
    info!("Configuration:");
    ...
    let bazelrc_path =
        std::env::temp_dir().join(format!("plasma-exec-{}.bazelrc", std::process::id()));
    let grpc_url_str = format!("grpc://127.0.0.1:{}", grpc_port);
    let bazelrc_content = format!(
        "# Auto-generated by Plasma exec\n\
