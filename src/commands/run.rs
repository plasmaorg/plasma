/// `plasma run` command implementation
///
...
use crate::cli_utils::plasma_prefix;
use crate::recipe::{
...
pub async fn run(args: &RunArgs) -> Result<()> {
    ...
    // Check if this is a local portable recipe (.js file without plasma shebang)
    // Portable recipes are executed with QuickJS runtime
    // If the .js file has a shebang like "#!/usr/bin/env -S plasma run node",
        ...
        .unwrap_or(false)
    {
        ...
        if !has_plasma_run_shebang(script_path)? {
    ...
    // Parse annotations
    if args.verbose {
        eprintln!("{} Parsing annotations from {}", plasma_prefix(), script);
    }
    ...
    if args.verbose {
        eprintln!("{} Cache key: {}", plasma_prefix(), cache_key);
    }
    ...
    if args.clean {
        if args.verbose {
            eprintln!("{} Cleaning cache for this script", plasma_prefix());
        }
    ...
    // Check cache
    if args.verbose {
        eprintln!("{} Checking cache...", plasma_prefix());
    }
    ...
    if let Some(entry) = cache.get(&cache_key)? {
        ...
        if args.verbose {
            eprintln!("{} Cache HIT ✓", plasma_prefix());
            eprintln!("{} Restoring outputs from cache", plasma_prefix());
    ...
    // Cache miss
    if args.verbose {
        eprintln!("{} Cache MISS ✗", plasma_prefix());
    }
    ...
    if result.exit_code == 0 {
        if args.verbose {
            eprintln!("{} Archiving outputs...", plasma_prefix());
        }
        ...
        if args.verbose {
            eprintln!("{} Cached as: {}", plasma_prefix(), cache_key);
        }
...
fn execute_script_no_cache(
    ...
    if verbose {
        ...
        eprintln!("{} Exit code: {}", plasma_prefix(), result.exit_code);
    }
...
/// Show cache status for a script (`plasma run --status script.sh`)
async fn run_status(args: &RunArgs, cache_dir: &std::path::Path) -> Result<()> {
    ...
    if let Some(entry) = cache.get(&cache_key)? {
        ...
        println!();
        println!("Run `plasma run {}` to cache this script.", script_path);
    }

    Ok(())
}

/// List all cached scripts (`plasma run --list`)
...
/// Show cache statistics (`plasma run --stats`)
...
async fn run_remote_recipe(recipe_ref: &str, args: &RunArgs) -> Result<()> {
    if args.verbose {
        eprintln!("{} Parsing remote recipe: {}", plasma_prefix(), recipe_ref);
    }
    ...
    if args.verbose {
        eprintln!("{} Fetching from {}", plasma_prefix(), remote.git_url());
    }
    ...
    if args.verbose {
        eprintln!("{} Executing recipe at root level", plasma_prefix());
    }
...
/// Check if a script file has a `plasma run <runtime>` shebang
///
/// This is used to distinguish between:
/// - Standard recipes: `#!/usr/bin/env -S plasma run node` -> use external node runtime
/// - Portable recipes: No plasma shebang -> use QuickJS runtime
fn has_plasma_run_shebang(script_path: &Path) -> Result<bool> {
    ...
    if let Some(Ok(first_line)) = reader.lines().next() {
        // Check for various plasma run shebang patterns
        // e.g., "#!/usr/bin/env -S plasma run node"
        // e.g., "#!/usr/bin/env plasma run python"
        if first_line.starts_with("#!") && first_line.contains("plasma run") {
            return Ok(true);
        }
    }
...
async fn run_local_portable_recipe(script_path: &Path, args: &RunArgs) -> Result<()> {
    ...
    if args.verbose {
        eprintln!("{} Executing recipe with QuickJS runtime", plasma_prefix());
    }
