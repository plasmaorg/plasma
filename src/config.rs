...
/// Upstream configuration (can be Plasma instance or storage backend)
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub struct TokenAuthConfig {
    /// Environment variable containing the token (defaults to PLASMA_TOKEN if not specified)
...
pub struct ServerConfig {
    ...
    /// When set to "regional", the server runs the Plasma protocol gRPC server
    #[serde(default)]
...
impl PlasmaConfig {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let config: PlasmaConfig = toml::from_str(&expanded_content)
    ...
    pub fn example_exec() -> String {
        let config = PlasmaConfig {
            ...
            upstream: vec![UpstreamConfig {
                url: "grpc://cache.example.com:7070".to_string(), // Plasma protocol
    ...
    pub fn example_server() -> String {
        let config = PlasmaConfig {
            cache: CacheConfig {
                dir: "/data/plasma/cache".to_string(),
            ...
            auth: AuthConfig {
                public_key_file: Some("/etc/plasma/jwt-public-key.pem".to_string()),
                public_key: None,
                jwks_url: None,
            ...
            server: ServerConfig {
                layer: Some("regional".to_string()), // Layer 2 runs Plasma protocol server
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(config.cache.dir, ".plasma/cache");
    ...
    fn test_invalid_eviction_policy() {
        let mut config = PlasmaConfig::default();
