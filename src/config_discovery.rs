...
/// Discovers Plasma configuration by traversing up the directory tree
pub fn discover_config(start_dir: &Path) -> Result<Option<PathBuf>> {
    let mut current = start_dir.to_path_buf();

    loop {
        let config_path = current.join("plasma.toml");
    ...
    if let Some(home) = dirs::home_dir() {
        let global_config = home.join(".config/plasma/config.toml");
...
pub fn load_config_with_discovery(
    ...
    if let Some(config_path) = explicit_path {
        ...
        if let Some(discovered_path) = discover_config(&current_dir)? {
            Ok(Some(PlasmaConfig::from_file(&discovered_path)?))
        } else {
            Ok(None)
        }
    }
...
impl DaemonState {
    ...
    /// Can be overridden with PLASMA_STATE_DIR for testing
    fn state_base_dir() -> PathBuf {
        if let Ok(state_dir) = std::env::var("PLASMA_STATE_DIR") {
    ...
    pub fn save(&self) -> Result<()> {
        ...
        // Write .plasma/bazelrc for automatic Bazel configuration
        self.write_bazelrc()
            .context("Failed to write .plasma/bazelrc")?;
    ...
    pub fn generate_env_exports(&self, shell: &str) -> String {
        ...
        match shell {
            "fish" => {
                ...
                exports.push(format!("set -gx PLASMA_HTTP_URL {}", http_url));
                exports.push(format!("set -gx PLASMA_GRPC_URL {}", grpc_url));
                exports.push(format!("set -gx PLASMA_CONFIG_HASH {}", self.config_hash));
                exports.push(format!("set -gx PLASMA_DAEMON_PID {}", self.pid));
                ...
                if let Some(ref socket) = self.unix_socket {
                    exports.push(format!("set -gx PLASMA_UNIX_SOCKET {}", socket.display()));
                }
            ...
            _ => {
                // bash/zsh
                // Plasma-specific variables
                exports.push(format!("export PLASMA_HTTP_URL={}", http_url));
                exports.push(format!("export PLASMA_GRPC_URL={}", grpc_url));
                exports.push(format!("export PLASMA_CONFIG_HASH={}", self.config_hash));
                exports.push(format!("export PLASMA_DAEMON_PID={}", self.pid));
                ...
                // Unix socket for Xcode
                if let Some(ref socket) = self.unix_socket {
                    exports.push(format!("export PLASMA_UNIX_SOCKET={}", socket.display()));
                }
...
pub fn populate_build_tool_env_vars(
    ...
    // Generic Plasma URLs
    env_vars.insert("PLASMA_HTTP_URL".to_string(), http_url.clone());
    env_vars.insert("PLASMA_GRPC_URL".to_string(), grpc_url);

    // Unix socket for Xcode (if available)
    if let Some(ref socket) = unix_socket {
        env_vars.insert(
            "PLASMA_UNIX_SOCKET".to_string(),
...
mod tests {
    ...
    fn test_discover_config_finds_nearest() {
        ...
        let config_path = project.join("plasma.toml");
    ...
    fn test_hash_config_is_consistent() {
        let temp = TempDir::new().unwrap();
        let config_path = temp.path().join("plasma.toml");
