...
//
// This module creates a QuickJS JavaScript runtime with Plasma APIs exposed.
...
/// Create a QuickJS runtime with Plasma APIs
///
/// The recipe_dir parameter is used to discover plasma.toml for configuration
pub async fn create_plasma_runtime_with_dir(
    recipe_dir: PathBuf,
) -> Result<(AsyncRuntime, AsyncContext)> {
    // Create runtime with module loader for LLRT modules + Plasma modules
    let resolver = BuiltinResolver::default()
        .with_module("fs")
        .with_module("fs/promises")
        .with_module("child_process")
        .with_module("path")
        .with_module("plasma:cache")
        .with_module("plasma:fs")
        .with_module("plasma:kv");
    ...
    // Register Plasma APIs
    async_with!(context => |ctx| {
        // Create Plasma global object
        let plasma = rquickjs::Object::new(ctx.clone())?;
        ...
        plasma.set("readFile", Function::new(ctx.clone(), Async(move |path: String| {
        ...
        plasma.set("writeFile", Function::new(ctx.clone(), Async(move |path: String, data: String| {
        ...
        plasma.set("exists", Function::new(ctx.clone(), Async(move |path: String| {
        ...
        plasma.set("glob", Function::new(ctx.clone(), Async(move |pattern: String| {
        ...
        plasma.set("exec", Function::new(ctx.clone(), Async(move |command: String, args: Option<Vec<String>>| {
        ...
        // Hashing
        plasma.set("hashFile", Function::new(ctx.clone(), Async(move |path: String| {
        ...
        plasma.set("cache", cache)?;

        // Set global
        ctx.globals().set("Plasma", plasma)?;
        ...
        // Register plasma:cache module
        Module::declare_def::<js_js_module_plasma_cache, _>(ctx.clone(), "plasma:cache")?;

        // Register plasma:fs module
        Module::declare_def::<js_js_module_plasma_fs, _>(ctx.clone(), "plasma:fs")?;

        // Register plasma:kv module
        Module::declare_def::<js_js_module_plasma_kv, _>(ctx.clone(), "plasma:kv")?;

        // Store working directory for cache APIs
        ctx.globals().set("__PLASMA_RECIPE_DIR__", recipe_dir_clone.to_string_lossy().to_string())?;
...
#[allow(dead_code)]
pub async fn create_plasma_runtime() -> Result<(AsyncRuntime, AsyncContext)> {
    create_plasma_runtime_with_dir(std::env::current_dir()?).await
}

// Module definitions for plasma:* modules

#[rquickjs::module]
mod js_module_plasma_cache {
    use super::*;
    ...
    pub async fn needs_run(ctx: Ctx<'_>, options: rquickjs::Object<'_>) -> JsResult<bool> {
        ...
        let working_dir_str: String = ctx.globals().get("__PLASMA_RECIPE_DIR__")?;
...
mod js_module_plasma_kv {
    ...
    pub async fn has(ctx: Ctx<'_>, key: String) -> JsResult<bool> {
        let working_dir_str: String = ctx.globals().get("__PLASMA_RECIPE_DIR__")?;
        let working_dir = PathBuf::from(working_dir_str);
        let cache_dir = working_dir.join(".plasma/cache");
...
mod tests {
    ...
    async fn test_create_runtime() {
        let result = create_plasma_runtime().await;
        assert!(result.is_ok());
    }

    #[tokio::test]
    async fn test_plasma_apis_available() {
        let (_runtime, context) = create_plasma_runtime().await.unwrap();

        async_with!(context => |ctx| {
            // Test that Plasma global is available
            let script = r#"
                typeof Plasma !== 'undefined' &&
                typeof Plasma.readFile === 'function' &&
                typeof Plasma.writeFile === 'function' &&
                typeof Plasma.exec === 'function' &&
                typeof Plasma.glob === 'function' &&
                typeof Plasma.hashFile === 'function' &&
                typeof Plasma.cache.get === 'function'
            "#;

            let result: bool = ctx.eval(script.as_bytes())?;
            assert!(result, "Plasma APIs should be available");
    ...
    async fn test_exec_command() {
        ...
        async_with!(context => |ctx| {
            let script = r#"
                (async () => {
                    const exitCode = await Plasma.exec("echo", ["hello"]);
                    return exitCode;
                })()
            "#;
    ...
    async fn test_file_operations() {
        ...
        let temp_file = temp_dir.join("plasma_test_file.txt");
        ...
        async_with!(context => |ctx| {
            ...
            let script = format!(r#"
                (async () => {{
                    const data = await Plasma.readFile("{}");
                    return data.length;
                }})()
            "#, temp_file_str);
