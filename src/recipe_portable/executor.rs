...
use super::runtime::create_plasma_runtime_with_dir;

/// Executes portable recipes (JavaScript files with Plasma APIs)
...
impl RecipeExecutor {
    ...
    pub async fn execute(&self) -> Result<()> {
        ...
        // Create QuickJS runtime with Plasma APIs
        let (_runtime, context) = create_plasma_runtime_with_dir(recipe_dir).await?;
...
mod tests {
    ...
    async fn test_execute_simple_recipe() {
        ...
        let recipe_code = r#"
            ...
            const exitCode = await Plasma.exec("echo", ["hello from recipe"]);
            if (exitCode !== 0) {
    ...
    async fn test_execute_file_operations() {
        ...
        let recipe_code = format!(
            ...
            const exists = await Plasma.exists("{}");
            if (!exists) {{
                throw new Error("File should exist");
            }}

            const files = await Plasma.glob("{}/*.txt");
            if (files.length === 0) {{
    ...
    async fn test_execute_root_level() {
        ...
        let recipe_code = r#"
            ...
            const files = await Plasma.glob("*.toml");
